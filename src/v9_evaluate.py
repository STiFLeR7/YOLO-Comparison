...
def evaluate_model():
    """Train and evaluate YOLOv9 model."""
    start_time = time.time()

    # Load and train the YOLOv9 model
    model = YOLO('YOLO-Comparison/yolo11n.pt')  # Ensure you have the YOLOv9 weights in the correct path
    results = model.train(data='D:/YOLO-Comparison/dataset/Vehicle_Detection_Image_Dataset/data.yaml', epochs=50, imgsz=640, project='results/v9')
    ...
    save_metrics_as_txt(metrics, 'results/v9/metrics.txt')
    ...
    plot_loss_graph(metrics, 'results/v9/loss_graph.png')
